...
coh <- read_csv("./SPL/spl_total_visit.csv") #8157
...
sink("./SPL/SPL_POST_PATIENT_NUM.txt",split=TRUE)
print("POST")
...
patient_post
patient_post %>% nrow()
patient_post$'Dx_location' <-case_when(patient_post$condition_concept_id %in% c(4167097, 37088427, 42492165, 42617470, 45548416, 37088436, 42492174, 42617479, 45582144) ~ "SPL_X",
                                      patient_post$condition_concept_id %in% c(36713107, 36713108, 4145719, 37088435, 42492173, 42617478, 45596576) ~ "SPL_L",
                                      patient_post$condition_concept_id %in% c(80497, 37088429, 42492167, 42617472, 45591834, 37088430, 42492168, 42617473, 45562721) ~ "SPL_C")                          
#patient$'Dx_location' <- as.factor(patient$'Dx_location')  # 서울대병원 협착증 코드 : 세종류 다 있음
dim(patient_post)[1] #6205

#patient_post %>% filter(Dx_location=="spl_X") %>% nrow() # 42331
...
patient_post <- patient_post %>% filter(!((person_id %in% more_loc_x$person_id)&(visit_occurrence_id %in% more_loc_x$visit_occurrence_id)&(Dx_location == "SPL_X")))
# 44213
...
patient_post %>% filter(((person_id %in% more_loc_x$person_id)&(visit_occurrence_id %in% more_loc_x$visit_occurrence_id)&(Dx_location == "SPL_X"))) %>% group_by(person_id,visit_occurrence_id,Dx_location) %>% dplyr::summarise(n=n()) %>% filter(n!=1) # 얘도 0인게 맞음
#patient_post %>% filter(((person_id %in% more_loc_x$person_id)&(visit_occurrence_id %in% more_loc_x$visit_occurrence_id)&(Dx_location == "SPL_X"))) %>% group_by(person_id,visit_occurrence_id,Dx_location) %>% dplyr::summarise(n=n()) %>% filter((person_id==72466)&(visit_occurrence_id==6576303))

patient_post

## 1차 수정 후에도 남아있는 SPL_X -> X_RAY 결과를 가지고 2차 수정
spl_x <- patient_post %>% filter(Dx_location == "SPL_X")
spl_x %>% nrow() # 41898
...
patient_post[(patient_post$person_id %in% spl_c$person_id)&(patient_post$visit_occurrence_id %in% spl_c$visit_occurrence_id),]$Dx_location = "SPL_C" 
patient_post[(patient_post$person_id %in% spl_l$person_id)&(patient_post$visit_occurrence_id %in% spl_l$visit_occurrence_id),]$Dx_location = "SPL_L"
...
sink("./SPL/SPL_POST_PATIENT_NUM.txt",append=TRUE,split=TRUE)
print("Dx_location")
patient_post$person_id %>% unique() %>% length() #1730명
patient_post %>% nrow() #6205
sink()
...
# 최종 데이터 저장_1
#####################################################################################################################
write_csv(patient_post,"./SPL/spl_total_post.csv")
...
write_csv(patient_post, "./SPL/spl_post_pt1.csv")
...
# 최종 데이터 저장
#####################################################################################################################
write_csv(patient_post,"./SPL/spl_total_post2.csv")
...
patient_post <- patient_post %>% select(-drug_exposure_start_date)
patient_post %>% nrow() #2490
write_csv(patient_post, "./SPL/spl_post_pt2.csv")
