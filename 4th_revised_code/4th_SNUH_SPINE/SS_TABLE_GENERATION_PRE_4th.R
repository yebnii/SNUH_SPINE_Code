...
coh <- read_csv("./SS/SS_total_visit.csv") #75813
nrow(coh) # 75813
...
sink("./SS/SS_PRE_PATIENT_NUM.txt",split=TRUE)
print("PRE")
patient_pre$person_id %>% unique() %>% length() #1730명
patient_pre %>% nrow() #6205
sink()
...
patient_pre
patient_pre %>% nrow()
patient_pre$'Dx_location' <-case_when(patient_pre$condition_concept_id == 77079 ~ "SS_X",
                                      patient_pre$condition_concept_id %in% c(4152592,136788) ~ "SS_L",
                                      patient_pre$condition_concept_id == 436785 ~ "SS_C")                          
#patient$'Dx_location' <- as.factor(patient$'Dx_location')  # 서울대병원 협착증 코드 : 세종류 다 있음
dim(patient_pre)[1] #44646

#patient_pre %>% filter(Dx_location=="SS_X") %>% nrow() # 42331
...
patient_pre <- patient_pre %>% filter(!((person_id %in% more_loc_x$person_id)&(visit_occurrence_id %in% more_loc_x$visit_occurrence_id)&(Dx_location == "SS_X")))
# 44213
...
patient_pre %>% filter(((person_id %in% more_loc_x$person_id)&(visit_occurrence_id %in% more_loc_x$visit_occurrence_id)&(Dx_location == "SS_X"))) %>% group_by(person_id,visit_occurrence_id,Dx_location) %>% dplyr::summarise(n=n()) %>% filter(n!=1) # 얘도 0인게 맞음
#patient_pre %>% filter(((person_id %in% more_loc_x$person_id)&(visit_occurrence_id %in% more_loc_x$visit_occurrence_id)&(Dx_location == "SS_X"))) %>% group_by(person_id,visit_occurrence_id,Dx_location) %>% dplyr::summarise(n=n()) %>% filter((person_id==72466)&(visit_occurrence_id==6576303))

patient_pre %>% nrow()

## 1차 수정 후에도 남아있는 SS_X -> X_RAY 결과를 가지고 2차 수정
SS_x <- patient_pre %>% filter(Dx_location == "SS_X")
SS_x %>% nrow() # 41898
...
patient_pre[(patient_pre$person_id %in% SS_c$person_id)&(patient_pre$visit_occurrence_id %in% SS_c$visit_occurrence_id),]$Dx_location = "SS_C" 
patient_pre[(patient_pre$person_id %in% SS_l$person_id)&(patient_pre$visit_occurrence_id %in% SS_l$visit_occurrence_id),]$Dx_location = "SS_L"
...
sink("./SS/SS_PRE_PATIENT_NUM.txt",append=TRUE,split=TRUE)
print("Dx_location")
patient_pre$person_id %>% unique() %>% length() #1730명
patient_pre %>% nrow() #6205
sink()
...
# 최종 데이터 저장_1
#####################################################################################################################
write_csv(patient_pre,"./SS/SS_total_pre.csv")
...
write_csv(patient_pre, "./SS/SS_pre_pt1.csv")
...
# 최종 데이터 저장
#####################################################################################################################
write_csv(patient_pre,"./SS/SS_total_pre2.csv")
...
write_csv(patient_pre, "./SS/SS_pre_pt2.csv")
